...
const C = {
  primary: rgb(0, 88 / 255, 188 / 255),       // #0058bc
  primaryLight: rgb(216 / 255, 226 / 255, 255 / 255), // #d8e2ff
  onSurface: rgb(25 / 255, 28 / 255, 29 / 255),        // #191c1d
  onSurfaceVariant: rgb(65 / 255, 71 / 255, 85 / 255), // #414755
  surface: rgb(248 / 255, 249 / 255, 250 / 255),       // #f8f9fa
  white: rgb(1, 1, 1),
  muted: rgb(113 / 255, 119 / 255, 134 / 255),         // #717786
...
export async function generateApplicationPDF(
  ...
  doc.setTitle(safeAscii(`Majimi - Application: ${program.title}`));
  doc.setAuthor("Majimi (maji-mi.app)");
  doc.setSubject(safeAscii(program.title));
  doc.setCreator("Majimi Mobile App");
  ...
  drawText(ctx, "MAJIMI (maji-mi.app)", MARGIN, PAGE_HEIGHT - 22, {
    font: boldFont,
    size: 11,
    color: C.white,
  });
  drawText(ctx, "Majimi - Application Auto-Generated Form", MARGIN, PAGE_HEIGHT - 38, {
    font: boldFont,
    size: 14,
    color: C.white,
  });
  ...
  lastPage.drawText(`Generated by Majimi (maji-mi.app) | ${dateStr} | For personal use only`, {
    x: MARGIN,
    y: 24,
    size: 7,
    font: regularFont,
  ...
  const safeTitle = program.title
    ...
    .replace(/\s+/g, "_")
    .substring(0, 40);
  const fileName = `majimi_application_${safeTitle}_${Date.now()}.pdf`;
...
export async function generateSimpleFormPDF(
  ...
  drawText(ctx, "MAJIMI - Application Pre-fill Sheet", MARGIN, PAGE_HEIGHT - 20, {
    font: boldFont,
    size: 13,
    color: C.white,
  });
  ...
  const today = new Date();
  const dateStr = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, "0")}-${String(today.getDate()).padStart(2, "0")}`;
  page.drawText(`Generated by Majimi | ${dateStr} | For personal use only`, {
    x: MARGIN,
    y: 24,
    size: 7,
    font: regularFont,
  ...
  const pdfBytes = await doc.save();
  const dirUri = FileSystem.documentDirectory ?? FileSystem.cacheDirectory ?? "";
  const fileName = `majimi_form_${program.id}_${Date.now()}.pdf`;
  const fileUri = `${dirUri}${fileName}`;
  const base64 = uint8ArrayToBase64(pdfBytes);
  await FileSystem.writeAsStringAsync(fileUri, base64, {
    encoding: FileSystem.EncodingType.Base64,
